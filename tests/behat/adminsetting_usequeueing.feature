...
Feature: Within a moodle instance, an administrator should be able to set the value for "Use queueing" for the entire Moodle installation.
  ...
  @javascript
  Scenario: Switch as an admin to the adminsettings of the module grouptool and change the value of "Use queueing" to "No". Then login as a teacher and add a new grouptool to a course and check whether the default value has changed.
    Given I log in as "admin"
    And I navigate to "Plugins > Activity modules > Grouptool" in site administration
    And I set the field "Use group size" to "1"
    And I set the field "Use queueing" to ""
    And I press "Save changes"
    And I log out
    And I log in as "teacher1"
    And I am on "Course 1" course homepage
